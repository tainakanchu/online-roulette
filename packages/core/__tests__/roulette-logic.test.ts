import { createDefaultRouletteLogic } from "../src/roulette";
...
describe("RouletteLogic", () => {
  ...
  describe("calculateSelectedIndex", () => {
    ...
    it("正しいインデックスを計算する", () => {
      ...
      expect(logic.calculateSelectedIndex(options, 0)).toBe(0); // 0度 -> 12時の位置
      expect(logic.calculateSelectedIndex(options, 90)).toBe(3); // 90度 -> 3時の位置
      expect(logic.calculateSelectedIndex(options, 180)).toBe(2); // 180度 -> 6時の位置
      expect(logic.calculateSelectedIndex(options, 270)).toBe(1); // 270度 -> 9時の位置
    });
    ...
    it("360度以上の回転でも正しく計算する", () => {
      ...
      expect(logic.calculateSelectedIndex(options, 360)).toBe(0); // 0度 -> 12時の位置
      expect(logic.calculateSelectedIndex(options, 450)).toBe(3); // 90度 -> 3時の位置
      expect(logic.calculateSelectedIndex(options, 540)).toBe(2); // 180度 -> 6時の位置
    });
    ...
    it("選択肢の数が変わっても正しく計算する", () => {
      ...
      // 2つの選択肢（180度ごと）
      expect(logic.calculateSelectedIndex(twoOptions, 0)).toBe(0); // 0度 -> 12時の位置
      expect(logic.calculateSelectedIndex(twoOptions, 180)).toBe(1); // 180度 -> 6時の位置

      // 6つの選択肢（60度ごと）
      expect(logic.calculateSelectedIndex(sixOptions, 0)).toBe(0); // 0度 -> 12時の位置
      expect(logic.calculateSelectedIndex(sixOptions, 60)).toBe(5); // 60度 -> 2時の位置
      expect(logic.calculateSelectedIndex(sixOptions, 120)).toBe(4); // 120度 -> 4時の位置
    });
  });
});
